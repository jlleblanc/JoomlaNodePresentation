...
chat_room.sockets.on('connection', function (socket) {
  ...
  joomla.auth_cookies(cookies, function  (j_user) {
    if (!j_user || j_user.username === "") {
      socket.emit('chat error', {message: 'Please log in to join the chat'});
    } else {
      chat_room.sockets.emit('enter chat', {message: j_user.username + ' has entered the chat.'});
      ...
      socket.on('disconnect', function  () {
        chat_room.sockets.emit('chat', {message: j_user.username + ' has disconected'});
      });
    }
  });
